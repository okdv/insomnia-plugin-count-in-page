...
const count = async context => {
  const countPrompt = await context.store.getItem(`count.${context.request.getId()}`);
  ...
  if (countPrompt && countPrompt.length > 0) {
    ...
    try {
      ...
      bodyArr.map((line, i) => {
        ...
        if (rgx.test(line)) {
          const replaced = line.replace(/\</g, "&lt;").replace(/\>/g, "&gt;").replace(regex, (match) => `<b style="color:red;">${match}</b>`);
          if (replaced !== line) {
      ...
      const html = `
        <h2>Found ${matches.length} instances of ${countPrompt} in ${context.request.getName()}:</h2>
